...
public class MCPRemap extends CUIBase {
	...
	List<RefOption> refOptsParsed = new ArrayList<>();
	...
	protected boolean checkOptions() throws Exception {
		...
		for (String s : refOpts) refOptsParsed.add(new RefOption(fromType, new File(s)));
		...
		for (String s : refnOpts) {
			...
			if (p.length != 2) {
				...
				try {
					refOptsParsed.add(new RefOption(NameSet.Type.valueOf(p[0]),
							new File(p[1])));
