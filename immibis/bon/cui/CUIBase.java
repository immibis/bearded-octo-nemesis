...
public abstract class CUIBase {
	...
	Field getOptionField(String name) {
		for (Field f : getClass().getFields()) {
			Option o = f.getAnnotation(Option.class);
			if (o != null && o.value().equals(name)) return f;
		}
		return null;
	}

	@SuppressWarnings("unchecked")
	boolean parseOptions(String[] args) throws Exception {
		if ((args.length % 2) == 1) {
		...
		for (int k = 0; k < args.length; k += 2) {
			...
			if (Enum.class.isAssignableFrom(f.getType())) {
				...
				if (f.get(this) != null) {
					System.err.println("Option specified more than once: " + opt);
					ok = false;
				} else f.set(this, new File(val));
			} else if (f.getType() == List.class) ((List<String>) f.get(this)).add(val); else {
	...
	protected void run(String[] args) throws Exception {
		if (args.length == 0) showUsage(); else if (parseOptions(args) && checkOptions()) run();
	}
