...
public class JarWriter {
	static void addDirectories(String filePath, Set<String> dirs) {
		int i = filePath.lastIndexOf('/');
		if (i >= 0) {
			String dirPath = filePath.substring(0, i);
			if (dirs.add(dirPath)) addDirectories(dirPath, dirs); // ensure all parent directories are added
		}
	}

	public static void write(File file, ClassCollection cc, IProgressListener progress) throws IOException {
		if (progress != null) progress.setMax(cc.getAllClasses().size() + cc.getExtraFiles().size());
		...
		try (JarOutputStream j_out = new JarOutputStream(new FileOutputStream(file))) {
			for (ClassNode cn : cc.getAllClasses()) {
				if (progress != null) progress.set(files++);
