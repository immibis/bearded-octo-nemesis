...
public class GuiMain extends JFrame {
	...
	// The Java Preferences API is used to store the last directory the user was
	// browsing
	...
	final Preferences prefs = Preferences.userNodeForPackage(GuiMain.class);
	final static String PREFS_KEY_BROWSEDIR = "browseDir";
	final static String PREFS_KEY_MCPDIR = "mcpDir";

	static String getPrintableStackTrace(Throwable e, Set<StackTraceElement> stopAt) {
		String s = e.toString();
		int numPrinted = 0;
		for (StackTraceElement ste : e.getStackTrace()) {
			boolean stopHere = false;
			if (stopAt.contains(ste) && numPrinted > 0) stopHere = true; else {
				s += "\n    at " + ste.toString();
				numPrinted++;
				if (ste.getClassName().startsWith("javax.swing.")) stopHere = true;
			}
	...
	static String getStackTraceMessage(Throwable e) {
		...
		while (e.getCause() != null) {
			Set<StackTraceElement> stopAt = new HashSet<StackTraceElement>(
	...
	JComboBox<Operation> opSelect;
	JComboBox<Side> sideSelect;
	JTextField inputField, outputField, mcpField;
	...
	final Reference<File> browseDir = new Reference<File>();

	// the last directory the user was browsing, for the MCP directory
	final Reference<File> mcpBrowseDir = new Reference<File>();
	...
	public GuiMain() {
...

		{
			...
			if (!mcpDirString.equals("")) mcpBrowseDir.val = new File(mcpDirString); else mcpBrowseDir.val = new File(".");
		}
	...
	synchronized void goButtonPressed() {
		if (curTask != null && curTask.isAlive()) return;
		...
		if (!mcpDir.isDirectory()) error = "MCP folder not found (at " + mcpDir + ")"; else if (!confDir.isDirectory()) error = "'conf' folder not found in MCP folder (at " + confDir + ")";
		...
		if (outputField.getText().equals("")) outputField.setText(inputField.getText() + op.defaultNameSuffix);
			...
			public void run() {
				...
				try {
					...
					for (String s : refPathList) {
						...
						refCCList.put(s, ClassCollectionFactory
								.loadClassCollection(refNS, refPathFile,
										progress));

						// progress.start(0, "Remapping "+s);
						// refs.add(Remapper.remap(mcpRefCC, inputNS,
						// Collections.<ClassCollection>emptyList(), progress));
					}
					...
					// remap to obf names from searge names, then searge names
					// to MCP names, in two steps
					// the first will be a no-op if the mod uses searge names
					// already
